...
import { User, Users, LogOut, Settings, ChevronDown, Compass, Bell, MessageSquareText, Briefcase } from "lucide-react"
...
export function Header() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 border-b border-border/30 bg-background/70 backdrop-blur-2xl">
      <div className={`flex h-12 items-center justify-between px-5 ${isPlanner ? "" : "mx-auto max-w-6xl"}`}>
        ...
        <div className="flex items-center gap-1 sm:gap-1.5">
          <Link to="/planner">
            <Button variant="ghost" size="sm" className={`gap-1.5 rounded-lg px-3 py-1.5 h-9 hover:bg-muted ${
              isPlanner ? "text-primary bg-primary/10 font-semibold" : "text-muted-foreground hover:text-foreground"
            }`}>
          ...
          <div className="relative" ref={communityRef}>
            <Button
              variant="ghost"
              size="sm"
              className={`gap-1.5 rounded-lg px-3 py-1.5 h-9 hover:bg-muted ${
                isCommunity ? "text-primary bg-primary/10 font-semibold" : "text-muted-foreground hover:text-foreground"
              }`}
            ...
            {communityOpen && (
              <div className="absolute left-0 top-full mt-2 w-44 overflow-hidden rounded-xl border border-border bg-card shadow-xl z-50">
                <Link
                  to="/community"
                  className="flex w-full items-center gap-2.5 px-4 py-2.5 text-[13px] font-medium text-foreground hover:bg-muted transition-colors"
                ...
                <Link
                  ...
                  onClick={() => setCommunityOpen(false)}
                >
                  <MessageSquareText className="h-4 w-4 text-nebula/60" />
                  자유게시판
                </Link>
              </div>
            )}
          ...
          {isAuthLoading ? (
            ...
            <Link to="/trips">
              <Button variant="ghost" size="sm" className={`gap-1.5 rounded-lg px-3 py-1.5 h-9 hover:bg-muted ${
                isTrips ? "text-primary bg-primary/10 font-semibold" : "text-muted-foreground hover:text-foreground"
              }`}>
            ...
            <Button
              size="sm"
              className="gap-1.5 rounded-lg text-[13px] font-semibold px-4 h-9"

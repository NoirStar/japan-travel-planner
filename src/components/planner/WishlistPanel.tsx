...
export function WishlistPanel({ open, onOpenChange, tripId, dayId }: WishlistPanelProps) {
  ...
  return (
    <>
      ...
      {/* 시트 패널 */}
      <div
        className="fixed inset-x-0 bottom-0 z-50 flex max-h-[75vh] flex-col rounded-t-2xl bg-card shadow-xl border-t border-border lg:left-0 lg:max-h-full lg:w-[400px] lg:rounded-none lg:rounded-tr-2xl"
          ...
          <div className="flex w-full items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              {visibleItems.length > 0 && (
                <span className="rounded-full bg-indigo/10 px-2 py-0.5 text-[10px] font-semibold text-primary">
                  {visibleItems.length}
                </span>
              )}
            </div>
        ...
        <div className="flex-1 overflow-y-auto px-3 py-2" data-testid="wishlist-items">
          {visibleItems.length === 0 ? (
            <div className="flex flex-col items-center gap-3 py-14 text-center">
              <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-indigo/10">
                ...
                return (
                  <div
                    ...
                    className={`flex items-center gap-2 rounded-lg border p-2 transition-all ${
                      isAdded ? "border-indigo/30 bg-indigo/5" : "border-border hover:border-primary/30 hover:shadow-sm"
                    }`}

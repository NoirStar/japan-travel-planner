...
**단계 검증 (Stage Review)**
- 다음 기능으로 넘어가기 **전에** 현재 단계의 전체 코드를 반드시 검증한다
- 검증 체크리스트:
  1. **구현 완성도**: 스펙 문서의 모든 요구사항이 빠짐없이 구현되었는가
  2. **성능**: 불필요한 리렌더링, 메모리 누수, 번들 사이즈 이슈는 없는가
  3. **유지보수성**: 단일 책임 원칙, 적절한 추상화, 명확한 네이밍, 중복 코드 제거
  4. **사용자 경험(UX)**: 반응형, 접근성(a11y), 로딩/에러 상태, 인터랙션 피드백
  5. **코드 품질**: 타입 안전성, 에러 핸들링, 엣지 케이스 처리
  6. **테스트 커버리지**: 핵심 시나리오 + 엣지 케이스가 테스트로 검증되는가
- 검증 후 발견된 문제는 **같은 브랜치에서 수정 후** PR 진행
- 검증 없이 다음 단계로 절대 넘어가지 않는다
